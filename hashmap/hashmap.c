...
unsigned int hash(char *key) 
{
	...
	for(i = strlen(key)-1;i>=0;i--) // loop
	{
		// calculations
		hashvalue += key[i]; 
		hashvalue = hashvalue % BINSIZE; // mod binsize	
	}
...
struct binEntry *lookup(char *key)
{
	...
	int hashvalue = hash(key); // key calculate

	struct binEntry* head = hashtable[hashvalue]; // temp head is initilaised

	while(head!=NULL) // loop
	{
		if(head->key==key) // if condition)
		{
			// return the pointer 
				return head; // return
			//return head->value;
...
{
	// YOUR CODE GOES HERE
	int hashvalue = hash(key); // getting the key
	struct binEntry* head = hashtable[hashvalue]; // allocating temp memory and initialising it	
	
	while(head!=NULL) // loop
	{
		//printf("check1");
		if(head->key==key) // condition check
		{
			if(head->prev==NULL) // if prev is NULL
			{
				hashtable[hashvalue] = head->next; // update with the next
			
			}
			else
			{
				// set prev to be head->next
				head->prev = head->next;
				//printf("2nd check\n");
			}
			char *value = head->value; // value deted
		...
		head->next = head->prev; // set next to prev
		head = head->next; // increment 
